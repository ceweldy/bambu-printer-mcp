...
export class STLManipulator extends EventEmitter {
  ...
  private async maybeFlattenBundle(
    ...
    if (flag === 'false' || flag === '0') return bundle;
    ...
    try {
      ...
      const isInstalledBblBundle = bundlePaths.every((bundlePath) =>
        bundlePath.includes(`${path.sep}profiles${path.sep}BBL${path.sep}`)
      );
      if (isInstalledBblBundle) {
        throw new Error(
          `Could not resolve installed Bambu profiles for safe CLI slicing: ${err?.message ?? err}`
        );
      }
      console.warn(
        `[cli-flatten] custom profile bundle was not flattened: ${err?.message ?? err}`
      );
      return bundle;
    }
  }
  ...
  private firstProfileValue(config: Record<string, any>, key: string): string | undefined {
  ...
  private async rewriteSuperTackSliceArchive(filePath: string): Promise<void> {
    ...
    const plateJsonEntries = Object.values(archive.files).filter(
      (entry) => !entry.dir && /^Metadata\/plate_\d+\.json$/i.test(entry.name)
    );
    const usedFilamentsByPlate = new Map<number, number[]>();
    ...
    const resolvePlateTemperature = (
      key: string,
      usedFilaments: number[]
    ): number | undefined => {
    ...
    const gcodeEntries = Object.values(archive.files).filter(
    ...
    for (const entry of gcodeEntries) {
      ...
      const initialTemperature =
        resolvePlateTemperature('supertack_plate_temp_initial_layer', usedFilaments) ??
      ...
      if (!initialTemperature || !normalTemperature) {
        throw new Error(
          `Selected filament profile does not declare usable SuperTack bed temperatures for plate ${plateNumber}.`
        );
      }

      let gcode = await entry.async('string');
      gcode = gcode.replace(
        /^;\s*curr_bed_type\s*=\s*.*$/m,
        '; curr_bed_type = Bambu Cool Plate SuperTack'
      );
      let currentLayer = 0;
      gcode = gcode
        .split(/\r?\n/)
        .map((line) => {
          const layerMatch = line.match(
            /^;\s*layer num\/total_layer_count:\s*(\d+)\s*\//
          );
          if (layerMatch) {
          ...
          const temperatureMatch = line.match(
            /^(\s*M(?:140|190)\s+S)(-?\d+(?:\.\d+)?)(.*)$/i
          );
          if (!temperatureMatch || Number(temperatureMatch[2]) <= 0) {
            return line;
          }
          const temperature =
            currentLayer >= 2 ? normalTemperature : initialTemperature;
  ...
  private async validateBambuSliceArchive(
    filePath: string,
    filamentPaths: string[],
    bedType?: string
  ): Promise<void> {
    if (process.env.BAMBU_CLI_VALIDATE_OUTPUT === '0') return;
    ...
    if (!projectEntry || gcodeEntries.length === 0) {
      throw new Error(
        'BambuStudio returned an incomplete 3MF without project settings and printable plate G-code.'
      );
    }

    const projectConfig = JSON.parse(await projectEntry.async('string'));
    const actualTypes = Array.isArray(projectConfig.filament_type)
      ? projectConfig.filament_type.map((value: unknown) => String(value).toUpperCase())
      : [];
    ...
    for (const filamentPath of filamentPaths) {
      ...
      if (requestedType && !actualTypes.includes(requestedType)) {
        throw new Error(
          `BambuStudio output filament mismatch: requested ${requestedType}, embedded ${actualTypes.join(', ') || 'none'}.`
        );
      }
      if (requestedName && !actualSettings.includes(requestedName)) {
        throw new Error(
          `BambuStudio output preset mismatch: requested "${requestedName}", embedded ${JSON.stringify(actualSettings)}.`
        );
      }
    }
    ...
    if (requestedBed && projectConfig.curr_bed_type !== requestedBed) {
      throw new Error(
        `BambuStudio output bed mismatch: requested "${requestedBed}", embedded "${projectConfig.curr_bed_type ?? 'unknown'}".`
      );
    }
    if (requestedBed === BAMBU_CLI_BED_TYPES.supertack_plate) {
      ...
      if (!sliceInfoEntry) {
        throw new Error(
          'BambuStudio output bed mismatch: SuperTack output is missing Metadata/slice_info.config.'
        );
      }
      const sliceInfo = await sliceInfoEntry.async('string');
      const plateBlocks =
        sliceInfo.match(/<plate\b[^>]*>[\s\S]*?<\/plate>/gi) ?? [];
      const inconsistentPlate = plateBlocks.find((plateBlock) => {
        const metadata = plateBlock.match(
          /<metadata\b(?=[^>]*\bkey="curr_bed_type")[^>]*\/>/i
        )?.[0];
      ...
      if (plateBlocks.length === 0 || inconsistentPlate) {
        throw new Error(
          'BambuStudio output bed mismatch: every slice_info plate must identify Bambu Cool Plate SuperTack.'
        );
      }
    }
  }
  ...
  async sliceSTL(
    ...
    const operationId = this.generateOperationId();
    this.activeOperations.set(operationId, true);
    const debugSlicer = process.env.BAMBU_MCP_DEBUG_SLICER === '1';
    const slicerLabel = path.basename(slicerPath);
    const inputLabel = path.basename(stlFilePath);
    const outputFileName = path.basename(stlFilePath, '.stl') + '.gcode';
    let outputFilePath = path.join(this.tempDir, outputFileName);

    const redactDiagnostic = (message: string, extraPaths: Array<string | undefined> = []): string => {
      if (debugSlicer) return message;
      ...
      for (const knownPath of knownPaths) {
        redacted = redacted.split(knownPath).join(`<${path.basename(knownPath) || 'local-path'}>`);
      }
    ...
    if (isPathLikeExecutable(slicerPath) && !fs.existsSync(slicerPath)) {
      this.activeOperations.delete(operationId);
      throw new Error(redactDiagnostic(`Slicer executable not found at: ${slicerPath}`));
    }
    if (slicerProfile && !fs.existsSync(slicerProfile)) {
        console.warn(
          redactDiagnostic(
            `Slicer profile specified but not found: ${slicerProfile}. Slicer might use defaults.`
          )
        );
    ...
    let bambuOutputValidation:
      | { filamentPaths: string[]; bedType?: string; rewriteSuperTack: boolean }
      | undefined;
    ...
    try {
      ...
      const safeExecutionMessage = `Executing slicer ${slicerLabel} for ${inputLabel}`;
      if (progressCallback) progressCallback(20, safeExecutionMessage);
      console.log(
        debugSlicer
          ? `Executing slicer: ${slicerPath} ${args.join(' ')}`
          : safeExecutionMessage
      );
        ...
        const process = execFile(slicerPath, args, (error, stdout, stderr) => {
          if (error) {
            ...
            if (debugSlicer) {
              ...
              console.error(`Slicer ${slicerLabel} failed${exitCode}.`);
            }
            reject(
              new Error(
                `Slicer ${slicerLabel} failed${exitCode}. ` +
                'Set BAMBU_MCP_DEBUG_SLICER=1 to include raw local paths and diagnostics.'
              )
            );
          } else {
            if (debugSlicer) {
              ...
              console.log(`Slicer ${slicerLabel} completed for ${inputLabel}.`);
            }
            resolve();
          }
        });
      ...
      if (!fs.existsSync(outputFilePath)) {
          throw new Error(
            redactDiagnostic(`Slicer finished but output file not found: ${outputFilePath}`)
          );
      }
      ...
      if (bambuOutputValidation) {
        await this.validateBambuSliceArchive(
          outputFilePath,
          bambuOutputValidation.filamentPaths,
          bambuOutputValidation.bedType
        );
      }
      ...
      const safeMessage = redactDiagnostic(rawMessage, args);
      console.error(`Slicing failed for ${inputLabel}: ${safeMessage}`);
      this.emit('operationError', {
        operationId,
        type: 'slice',
        error: safeMessage
