...
import {
  ...
  type BambuNetworkBridgeOptions,
} from "./bambu-network-bridge.js";
...
import {
  PrinterRegistry,
  SUPPORTED_BAMBU_MODELS,
  type ResolvedPrinterProfile,
...
const VALID_BAMBUSTUDIO_CLI_BED_TYPES = [...VALID_BED_TYPES] as const;
...
function resolveBambuStudioCliBedType(argsBedType: string | undefined): string {
  ...
  if (!(VALID_BAMBUSTUDIO_CLI_BED_TYPES as readonly string[]).includes(bedType)) {
    throw new Error(
      `Unsupported BambuStudio CLI bed_type "${bedType}". Choose one of: ${VALID_BAMBUSTUDIO_CLI_BED_TYPES.join(", ")}`
    );
  }
  return bedType;
}
...
function readRuntimeConfig(): RuntimeConfig {
  ...
  if (
    ...
    throw new Error(
      "Streamable HTTP is restricted to a loopback host because this server does not provide remote authentication."
    );
  }
...
function printerTargetProperty() {
  return {
    type: "string",
    description:
      "Configured printer ID. Omit to use the default printer or the only configured printer.",
  };
}
...
class BambuPrinterMCPServer {
  ...
  private resolvePrinterContext(args: Record<string, any>): ResolvedPrinterProfile {
    const requestedModel = String(
      args.bambu_model || (this.printerRegistry.size === 0 ? DEFAULT_BAMBU_MODEL : "") || ""
    ).trim();
    ...
    if (this.printerRegistry.size > 0) {
      const profile = this.printerRegistry.resolve(printerId, {
        ...
        nozzleDiameter:
          typeof args.nozzle_diameter === "string" ? args.nozzle_diameter : undefined,
      });
      const protectedOverrides: Array<[unknown, string, string]> = [
      ...
      if (conflicts.length > 0) {
        throw new Error(
          `Configured printer connection fields cannot be overridden: ${conflicts.join(", ")}.`
        );
      }
      return profile;
    }
    ...
    return this.printerRegistry.resolve(printerId, {
      ...
      serial:
        typeof args.bambu_serial === "string" ? args.bambu_serial : DEFAULT_BAMBU_SERIAL,
      accessToken:
        typeof args.bambu_token === "string" ? args.bambu_token : DEFAULT_BAMBU_TOKEN,
  ...
  private applyPrinterContext(args: Record<string, any>, profile: ResolvedPrinterProfile): void {
    args.host ??= profile.host;
    args.bambu_serial ??= profile.serial;
    args.bambu_token ??= profile.accessToken;
    args.bambu_model ??= profile.model;
    args.dev_id ??= profile.devId;
    args.bed_type ??= profile.bedType;
    args.nozzle_diameter ??= profile.nozzleDiameter;
  }

  private async getFleetStatus(concurrency: number): Promise<Record<string, unknown>> {
    ...
    const results: Array<Record<string, any>> = new Array(summaries.length);
    let nextIndex = 0;
    ...
    const worker = async () => {
      while (nextIndex < summaries.length) {
        ...
        results[index] = await this.printerRegistry.runExclusive(summary.id, async () => {
          let profile: ResolvedPrinterProfile;
          try {
            ...
            return {
              ...
              status: "not_ready",
              error:
                summary.missing_environment_variables.length > 0
          ...
          try {
            const status = await this.bambu.getStatus(
              profile.host,
              profile.serial,
              profile.accessToken
            );
    ...
    await Promise.all(
      Array.from({ length: Math.min(boundedConcurrency, Math.max(1, summaries.length)) }, worker)
    );
  ...
  private bridgeOptionsFromArgs(args: Record<string, any> | undefined): BambuNetworkBridgeOptions {
    return {
      ...
      confirmationTimeoutMs:
        args?.confirmation_timeout_ms !== undefined
  ...
  private async print3mfViaBambuNetwork(
    ...
    const requiresLanMqttConnection =
      requiresBambuNetworkProjectFileAcknowledgement(bridgePrintMethod);
    ...
    const submissionNames = createBambuNetworkSubmissionNames(
      projectName,
      args?.task_name !== undefined ? String(args.task_name) : undefined
    );
    ...
    const { bridgeResult, printerAcknowledgement, printerJob } =
      await this.bambuNetwork.withExclusiveLifecycle(async () => {
        if (requiresLanMqttConnection) {
          ...
          if (!this.bambuNetwork.supportsProjectFileSequenceCorrelation()) {
            throw new Error(
              "The BambuNetwork runtime does not advertise project_file sequence correlation. " +
              "Apply the bundled host and open-bambu-networking patches before submitting a local print."
            );
          }
        }

        const currentBridgeResult = await this.bambuNetwork.callWithAgent(
          bridgeMethod,
        ...
        if (
          typeof currentBridgeResult === "object" &&
          currentBridgeResult !== null &&
          (currentBridgeResult as { ok?: unknown }).ok === false
        ) {
          throw new Error(
            `FULU BambuNetwork bridge method ${bridgeMethod} failed: ${String(
              (currentBridgeResult as { error?: unknown }).error || "unknown bridge error"
            )}`
          );
        }
        if (
          typeof currentBridgeResult === "object" &&
          currentBridgeResult !== null &&
          typeof (currentBridgeResult as { value?: unknown }).value === "number" &&
          (currentBridgeResult as { value: number }).value !== 0
        ) {
          const value = (currentBridgeResult as { value: number }).value;
          throw new Error(`FULU BambuNetwork bridge method ${bridgeMethod} returned non-zero result ${value}.`);
        }

        let acknowledgement: unknown;
        let printerJob: unknown;
        if (requiresBambuNetworkProjectFileAcknowledgement(bridgePrintMethod)) {
          const projectFileSequenceId =
            typeof currentBridgeResult === "object" &&
            currentBridgeResult !== null &&
            ["string", "number"].includes(
              typeof (currentBridgeResult as { project_file_sequence_id?: unknown })
                .project_file_sequence_id
            )
              ? String(
                  (currentBridgeResult as { project_file_sequence_id: string | number })
                    .project_file_sequence_id
                ).trim()
              : "";
          acknowledgement =
            await this.bambuNetwork.waitForLocalPrintAcknowledgement(
              devId,
              projectFileSequenceId,
              bridgeOptions
            );
            printerJob = await this.bambuNetwork.waitForPrinterJobStart(
              devId,
              [submissionName, submissionTaskName],
              bridgeOptions
            );
        }
        return {
  ...
  setupResourceHandlers() {
    ...
    this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
      ...
      const targetId =
        printerId === "default" && !this.printerRegistry.has("default")
      ...
      const content = await this.printerRegistry.runExclusive(targetId, async () => {
        ...
        if (resource === "hms") {
          ...
          if (!diagnostics.hms && status.connected) {
            await new Promise((r) => setTimeout(r, 1500));
            const retryStatus = await this.bambu.getStatus(
              profile.host,
              profile.serial,
              profile.accessToken
            );
  ...
  setupToolHandlers() {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      for (const tool of tools) {
        if (!PRINTER_CONTEXT_TOOLS.has(tool.name)) continue;
        tool.inputSchema.properties ??= {};
        tool.inputSchema.properties.printer ??= printerTargetProperty();
        if (
          tool.name !== "slice_stl" &&
          ...
          tool.inputSchema.required = tool.inputSchema.required.filter(
            (field: string) => field !== "bambu_model"
          );
        }
      }
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      const implicitOptionalTarget =
        optionalPrinterContext && typeof args.printer !== "string"
          ? this.printerRegistry.implicitReadyTargetId()
          : undefined;
      const shouldResolvePrinter =
        PRINTER_CONTEXT_TOOLS.has(name) &&
        (
      ...
      try {
        ...
        const executeTool = async () => {
          ...
          case "add_printer": {
            ...
            const profileInput = {
              ...
              accessToken:
                args.access_token !== undefined ? String(args.access_token) : undefined,
              accessTokenEnv:
                args.access_token_env !== undefined
              ...
              nozzleDiameter:
                args.nozzle_diameter !== undefined
                  ? String(args.nozzle_diameter)
                  : undefined,
              default: args.make_default === true,
            };
            const addProfile = async () => {
              let previous: ResolvedPrinterProfile | undefined;
              ...
              const added = this.printerRegistry.add(
                profileInput,
                { persist: args.persist === true, replace: args.replace === true }
              );
              if (previous) {
                await this.bambu.disconnectPrinter(
                  previous.host,
                  previous.serial,
          ...
          case "remove_printer": {
            ...
            const removed = await this.printerRegistry.runExclusive(printerId, async () => {
              let profile: ResolvedPrinterProfile | undefined;
              try {
          ...
          case "reconnect_printer": {
            ...
            const reconnectResults = await Promise.all(
              targetIds.map((targetId) =>
                this.printerRegistry.runExclusive(targetId, async () => {
                  let profile: ResolvedPrinterProfile;
                  try {
              ...
              result = await this.bambuNetwork.withExclusiveLifecycle(async () => {
                ...
                if (Boolean(bridgeArgs?.probe_printer)) {
                  ...
                  const accessCode = String(
                    bridgeArgs?.password || bridgeArgs?.bambu_token || bambuToken || ""
                  ).trim();
                  if (!devId || !devIp || !accessCode) {
                    throw new Error(
                      "probe_printer requires a selected printer with device id, IP address, and access code."
                    );
                  }
                  await this.bambuNetwork.connectPrinter(
                    {
                      ...
                      accessCode,
                      useSsl:
                        bridgeArgs?.use_ssl_for_mqtt !== undefined
          ...
          case "bambu_network_call": {
            ...
            if (isManagedBambuNetworkCallMethod(method)) {
              throw new Error(
                `BambuNetwork method ${method} is managed by the readiness and print tools so its connection and acknowledgement lifecycle cannot be bypassed.`
              );
            }
            const call = async () =>
              method === "bridge.poll_events"
                ? await this.bambuNetwork.pollEventsForControl(payload, options)
                : requiresBambuNetworkAgentForRawCall(
                    method,
                    bridgeArgs.with_agent
                  )
            ...
            result =
              ...
              this.bambuNetwork.isRawControlLaneOpen()
                ? await call()
                : await this.bambuNetwork.withExclusiveLifecycle(
                    call,
                    { allowRawControl: true }
                  );
            break;
          ...
          case "load_ams_filament":
            ...
            result = await this.bambu.loadAmsFilament(
              host,
              bambuSerial,
              bambuToken,
              Number(args.slot),
              Number(args.target_temperature)
            );
            break;
          ...
          case "print_3mf": {
            ...
            try {
              ...
              throw new Error(
                `Could not validate or auto-slice the 3MF: ${sliceCheckErr?.message ?? sliceCheckErr}`
              );
            }
        ...
        if (shouldResolvePrinter) {
          const targetId = implicitOptionalTarget ?? this.printerRegistry.targetId(
            typeof args.printer === "string" ? args.printer : undefined
          );
