 ...
 * BAMBU_CLIENT_KEY paths, or from the private application config directory.
 * Returns null if files are missing, so the caller falls back to no-cert TLS.
 */
export function loadClientCreds(): { cert: Buffer; key: Buffer } | null {
  const explicitCertPath = process.env.BAMBU_CLIENT_CERT;
  const explicitKeyPath = process.env.BAMBU_CLIENT_KEY;
  if (Boolean(explicitCertPath) !== Boolean(explicitKeyPath)) return null;
  const configDirectory = path.join(os.homedir(), ".config", "bambu-printer-mcp");
  const certPath = explicitCertPath || path.join(configDirectory, "client.crt");
  const keyPath = explicitKeyPath || path.join(configDirectory, "client.key");
  if (
    !explicitCertPath &&
    (!existsSync(certPath) || !existsSync(keyPath))
  ) {
    const legacyDirectory = path.join(os.homedir(), "Desktop", "bambu certs");
    const legacyCertPath = path.join(legacyDirectory, "embedded-cert.pem");
    const legacyKeyPath = path.join(legacyDirectory, "embedded-key.pem");
    if (existsSync(legacyCertPath) && existsSync(legacyKeyPath)) {
      ...
      try {
        ...
        writeFileSync(certPath, readFileSync(legacyCertPath), { mode: 0o600, flag: "wx" });
        wroteCert = true;
        writeFileSync(keyPath, readFileSync(legacyKeyPath), { mode: 0o600, flag: "wx" });
        wroteKey = true;
        chmodSync(certPath, 0o600);
        chmodSync(keyPath, 0o600);
      } catch {
        try {
          if (wroteCert && existsSync(certPath)) unlinkSync(certPath);
          if (wroteKey && existsSync(keyPath)) unlinkSync(keyPath);
        } catch {
          // The in-memory legacy fallback still keeps the current process usable.
        }
        try {
          return { cert: readFileSync(legacyCertPath), key: readFileSync(legacyKeyPath) };
        } catch {
          return null;
        }
      }
...
export function createSignedPrintEnvelope(
...
export function requiresEncryptedGcodeLine(printerSerial: string): boolean {
...
function getClientCertId(cert: Buffer, printerSerial: string): string {
...
function hasOperationalStatus(report: Record<string, any> | null | undefined): boolean {
  return typeof report?.gcode_state === "string" && report.gcode_state.trim().length > 0;
}
...
function getPrinterKey(host: string, serial: string, token: string): string {
  ...
  return `${host}-${serial}-${credentialFingerprint}`;
}
...
class TolerantBambuClient extends BambuClient {
  ...
  override publish(payload: any): Promise<void> {
    if (
      CLIENT_CREDS &&
      payload &&
      ...
      if (payload.print.command === "gcode_line") {
        if (requiresEncryptedGcodeLine(this.config.serialNumber)) {
          return Promise.reject(
            new Error(
              "This printer requires printer-key param_enc encryption for gcode_line commands, which is not yet available."
            )
          );
        }
        return super.publish(payload);
      }
      const certId = getClientCertId(
        CLIENT_CREDS.cert,
        this.config.serialNumber
      );
      return super.publish(
        createSignedPrintEnvelope(payload, CLIENT_CREDS.key, certId)
      );
    }
    return super.publish(payload);
  }
...
class BambuClientStore {
  ...
  private connectingPrinters: Map<string, BambuClient> = new Map();
  private connectionEpochs: Map<string, number> = new Map();
  ...
  private clearPrinterState(key: string, expectedEpoch?: number): void {
    if (
      expectedEpoch !== undefined &&
      this.connectionEpochs.get(key) !== expectedEpoch
    ) {
      return;
    }
    this.printers.delete(key);
    this.connectingPrinters.delete(key);
  ...
  async waitForInitialReport(
    ...
    if (!existing || Object.keys(existing).length === 0) {
      ...
      try {
        ...
        console.warn("No initial printer report received:", error);
      }
    }

    // The first MQTT push can be a sparse "hello" with only model/modules.
    // Wait within the original bounded timeout for an operational status
    // packet so fleet and reconnect responses do not briefly report UNKNOWN.
    let snapshot = this.reportSnapshots.get(key) || null;
    while (!hasOperationalStatus(snapshot) && Date.now() < deadline) {
      await sleep(Math.min(100, Math.max(1, deadline - Date.now())));
      snapshot = this.reportSnapshots.get(key) || null;
    }

    if (snapshot && !hasOperationalStatus(snapshot)) {
      console.warn("Initial printer report remained sparse until the status timeout.");
    }
    return snapshot;
  }
  ...
  async getPrinter(host: string, serial: string, token: string): Promise<BambuClient> {
    ...
    if (this.initialConnectionPromises.has(key)) {
      ...
      throw new Error("Existing Bambu client connection attempt failed.");
    }
    ...
    const epoch = (this.connectionEpochs.get(key) ?? 0) + 1;
    this.connectionEpochs.set(key, epoch);
    this.connectingPrinters.set(key, printer);
    this.ensureInitialReportPromise(key);

    printer.on("rawMessage", (_topic, payload) => {
      if (this.connectionEpochs.get(key) !== epoch) return;
      try {
    ...
    printer.on("client:connect", () => {
      if (this.connectionEpochs.get(key) !== epoch) {
        void printer.disconnect().catch(() => undefined);
        return;
      }
    ...
    printer.on("client:error", () => {
      this.clearPrinterState(key, epoch);
    });
    ...
    const connectPromise = Promise.resolve().then(() => printer.connect()).then(() => {});
    this.initialConnectionPromises.set(key, connectPromise);

    try {
      await connectPromise;
      if (this.connectionEpochs.get(key) !== epoch) {
        throw new Error("Bambu client connection was superseded by a reconnect or removal.");
      }
      this.printers.set(key, printer);
      this.connectingPrinters.delete(key);
      this.initialConnectionPromises.delete(key);
  ...
  async disconnectAll(): Promise<void> {
    ...
    const clients = new Set([
      ...
      ...this.connectingPrinters.values(),
    ]);
    for (const key of new Set([
      ...this.printers.keys(),
      ...this.connectingPrinters.keys(),
      ...this.initialConnectionPromises.keys(),
    ])) {
      this.connectionEpochs.set(key, (this.connectionEpochs.get(key) ?? 0) + 1);
    }

    for (const printer of clients) {
  ...
  async disconnect(host: string, serial: string, token: string): Promise<void> {
    const exactKey = getPrinterKey(host, serial, token);
    const pendingConnection = this.initialConnectionPromises.get(exactKey);
    const printer = this.printers.get(exactKey) ?? this.connectingPrinters.get(exactKey);
    this.connectionEpochs.set(exactKey, (this.connectionEpochs.get(exactKey) ?? 0) + 1);
    try {
      if (printer) {
        try {
          await printer.disconnect();
        } catch {
          console.warn("Bambu client disconnect failed; cached connection state was cleared.");
        }
      }
      ...
      this.clearPrinterState(exactKey);
    }
  }
}
...
export class BambuImplementation {
  ...
  private async getIdlePrinter(
    ...
    if (!idleStates.has(currentState)) {
      throw new Error(
        `${action} is allowed only while the printer is idle; current print state is ${currentState || "UNKNOWN"}.`
      );
    }
    return printer;
  }

  private async publishPrintCommandAndWait(
    printer: BambuClient,
    payload: Record<string, any>,
    timeoutMs = 5000
  ): Promise<Record<string, any> | null> {
    const eventPrinter = printer as any;
    if (
      typeof eventPrinter.on !== "function" ||
    ...
    return new Promise<Record<string, any>>((resolve, reject) => {
      ...
      const onRawMessage = (_topic: string, body: Buffer | string) => {
        ...
        if (
          ...
          finish(
            new Error(
              `Printer rejected ${command}${details.length > 0 ? `: ${details.join(" / ")}` : "."}`
            )
          );
          return;
        }
  ...
  private async publishProjectFileAndWait(
    ...
    const previousFile = String(
      eventPrinter.data?.subtask_name ?? eventPrinter.data?.gcode_file ?? ""
    );
    ...
    return new Promise<Record<string, any>>((resolve, reject) => {
      ...
      const describeFailure = (response: Record<string, any>): Error => {
        const details = [
          response.reason,
        ...
        const suffix = verificationFailed
          ...
          : "";
        return new Error(
          `Printer rejected the project file${details.length > 0 ? `: ${details.join(" / ")}.` : "."}${suffix}`
        );
      };
      const onMessage = (_topic: string, category: string, data: Record<string, any>) => {
        if (
          category !== "print" ||
      ...
      const timeout = setTimeout(() => {
        ...
        const fileMatches =
          currentFileLower.length > 0 &&
          (currentFileLower.includes(expectedFileLower) ||
            expectedFileLower.includes(currentFileLower));
        const attributableTransition =
          currentState !== previousState || currentFile !== previousFile;
        const acceptedByState =
          ["PREPARE", "SLICING", "RUNNING", "PAUSE"].includes(currentState) &&
          fileMatches &&
        ...
        finish(
          new Error(
            "Printer did not acknowledge the project_file command or enter an active print state."
          )
        );
      }, timeoutMs);
  ...
  async getStatus(host: string, serial: string, token: string): Promise<any> {
    try {
      ...
      console.error("Failed to get Bambu status:", error);
  ...
  async print3mf(
    ...
    if (isH2) {
      ...
      if (baseMapping.length > 5) {
        throw new Error(
          `P1/A1/X1 project_file supports at most 5 project filament positions; got ${baseMapping.length}.`
        );
      }
    ...
    const acceptance = await this.publishProjectFileAndWait(
      printer,
      projectFileCmd,
      remoteFileName
    );
  ...
  async resetAms(host: string, serial: string, token: string): Promise<any> {
  ...
  async loadAmsFilament(
    ...
    if (
      !Number.isFinite(normalizedTemperature) ||
    ...
    const printer = await this.getIdlePrinter(
      host,
      serial,
      token,
      "AMS filament loading"
    );

    const currentTemperature = Math.max(
      0,
      Math.trunc(Number((printer as any).data?.nozzle_temper ?? 0))
    );
  ...
  async unloadAmsFilament(host: string, serial: string, token: string): Promise<any> {
    const printer = await this.getIdlePrinter(
      host,
      serial,
      token,
      "AMS filament unloading"
    );
  ...
  async rebootPrinter(host: string, serial: string, token: string): Promise<any> {
  ...
  async cameraSnapshot(
    ...
    // `ipcam.rtsp_url` for these models, and tested H2-series firmware rejects the
  ...
  async disconnectPrinter(host: string, serial: string, token: string): Promise<void> {
    await this.printerStore.disconnect(host, serial, token);
  }
}
