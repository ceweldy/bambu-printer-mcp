...
import { ListResourcesRequestSchema, ListResourceTemplatesRequestSchema, ReadResourceRequestSchema, ListToolsRequestSchema, CallToolRequestSchema, ErrorCode, McpError } from "@modelcontextprotocol/sdk/types.js";
...
import { BambuNetworkBridge, createBambuNetworkSubmissionNames, isBambuNetworkControlMethod, isManagedBambuNetworkCallMethod, requiresBambuNetworkAgentForRawCall, requiresBambuNetworkProjectFileAcknowledgement, } from "./bambu-network-bridge.js";
import { hasAmsMappingInput, normalizeAmsMappingObject, normalizeBridgeAmsTrayValue } from "./ams-mapping.js";
import { analyze3MFAmsRequirements, analyze3MFPlateObjects, extractBambuTemplateSettings, parse3MF } from './3mf_parser.js';
import { BambuImplementation } from "./printers/bambu.js";
import { PrinterRegistry, SUPPORTED_BAMBU_MODELS, } from "./printer-registry.js";
import { redactPrinterConnectionError } from "./redaction.js";
dotenv.config();
...
function readRuntimeConfig() {
    ...
    if (transport === "streamable-http" &&
        !["127.0.0.1", "localhost", "::1"].includes(httpHost.toLowerCase())) {
        throw new Error("Streamable HTTP is restricted to a loopback host because this server does not provide remote authentication.");
    }
    return {
        transport,
        httpHost,
...
const PRINTER_CONTEXT_TOOLS = new Set([
    "get_printer_status",
...
function printerTargetProperty() {
    return {
        type: "string",
        description: "Configured printer ID. Omit to use the default printer or the only configured printer.",
    };
}
class BambuPrinterMCPServer {
    constructor() {
        ...
        this.server = new Server({
            ...
            version: PACKAGE_METADATA.version || "0.0.0"
        }, {
            capabilities: {
    ...
    resolvePrinterContext(args) {
        const requestedModel = String(args.bambu_model || (this.printerRegistry.size === 0 ? DEFAULT_BAMBU_MODEL : "") || "").trim();
        const validatedModel = requestedModel ? validateBambuModel(requestedModel) : undefined;
        const printerId = typeof args.printer === "string" ? args.printer : undefined;
        if (this.printerRegistry.size > 0) {
            const profile = this.printerRegistry.resolve(printerId, {
                bedType: typeof args.bed_type === "string" ? args.bed_type : undefined,
                nozzleDiameter: typeof args.nozzle_diameter === "string" ? args.nozzle_diameter : undefined,
            });
            const protectedOverrides = [
                [args.host, profile.host, "host"],
                [args.bambu_serial, profile.serial, "bambu_serial"],
                [args.bambu_token, profile.accessToken, "bambu_token"],
                [args.dev_id, profile.devId, "dev_id"],
                [validatedModel, profile.model, "bambu_model"],
            ];
            const conflicts = protectedOverrides
                .filter(([candidate, expected]) => candidate !== undefined && String(candidate) !== expected)
                .map(([, , field]) => field);
            if (conflicts.length > 0) {
                throw new Error(`Configured printer connection fields cannot be overridden: ${conflicts.join(", ")}.`);
            }
            return profile;
        }
        return this.printerRegistry.resolve(printerId, {
            host: typeof args.host === "string" ? args.host : DEFAULT_HOST,
            serial: typeof args.bambu_serial === "string" ? args.bambu_serial : DEFAULT_BAMBU_SERIAL,
            accessToken: typeof args.bambu_token === "string" ? args.bambu_token : DEFAULT_BAMBU_TOKEN,
            model: validatedModel,
            devId: typeof args.dev_id === "string" ? args.dev_id : DEFAULT_BAMBU_DEV_ID,
            bedType: typeof args.bed_type === "string" ? args.bed_type : undefined,
            nozzleDiameter: typeof args.nozzle_diameter === "string" ? args.nozzle_diameter : undefined,
        });
    }
    applyPrinterContext(args, profile) {
        args.host ?? (args.host = profile.host);
        args.bambu_serial ?? (args.bambu_serial = profile.serial);
        args.bambu_token ?? (args.bambu_token = profile.accessToken);
        args.bambu_model ?? (args.bambu_model = profile.model);
        args.dev_id ?? (args.dev_id = profile.devId);
        args.bed_type ?? (args.bed_type = profile.bedType);
        args.nozzle_diameter ?? (args.nozzle_diameter = profile.nozzleDiameter);
    }
    async getFleetStatus(concurrency) {
        const boundedConcurrency = Math.max(1, Math.min(8, Math.trunc(concurrency || 3)));
        const summaries = this.printerRegistry.list();
        if (summaries.length === 0) {
            return { status: "offline", total: 0, online: 0, printers: [] };
        }
        ...
        const worker = async () => {
            while (nextIndex < summaries.length) {
                ...
                results[index] = await this.printerRegistry.runExclusive(summary.id, async () => {
                    let profile;
                    try {
                        profile = this.printerRegistry.resolve(summary.id);
                    }
                    catch {
                        return {
                            printer: summary.id,
                            ...
                            connected: false,
                            status: "not_ready",
                            error: summary.missing_environment_variables.length > 0
                                ? `Missing environment variables: ${summary.missing_environment_variables.join(", ")}`
                                : "Printer credentials are incomplete.",
                        };
                    }
                    try {
                        const status = await this.bambu.getStatus(profile.host, profile.serial, profile.accessToken);
                        return {
                            printer: profile.id,
                            name: profile.name,
                            model: profile.model,
                            ...
                            print: status?.print ?? null,
                            error: status?.error ? redactPrinterConnectionError(status.error) : null,
                        };
                    }
                    catch (error) {
                        return {
                            ...
                            connected: false,
                            status: "error",
                            error: redactPrinterConnectionError(error),
                        };
                    }
                });
            }
        };
        await Promise.all(Array.from({ length: Math.min(boundedConcurrency, Math.max(1, summaries.length)) }, worker));
        return {
            status: results.every((entry) => entry.connected)
                ? "online"
                : results.some((entry) => entry.connected)
                    ? "partial"
                    : "offline",
            total: results.length,
            online: results.filter((entry) => entry.connected).length,
            printers: results,
        };
    }
    setupHandlers() {
    ...
    bridgeOptionsFromArgs(args) {
        return {
            ...
            certDir: args?.bambu_network_cert_dir !== undefined ? String(args.bambu_network_cert_dir) : undefined,
            ...
            confirmationTimeoutMs: args?.confirmation_timeout_ms !== undefined
                ? Number(args.confirmation_timeout_ms)
                : undefined,
        };
    }
    ...
    async print3mfViaBambuNetwork(args, host, bambuSerial, bambuToken) {
        ...
        const requiresLocalAccess = bridgePrintMethod !== "start_print";
        const requiresLanMqttConnection = requiresBambuNetworkProjectFileAcknowledgement(bridgePrintMethod);
        ...
        const password = requiresLocalAccess ? (explicitPassword || String(bambuToken || "").trim()) : explicitPassword;
        if (requiresLocalAccess && (!devIp || devIp === "localhost")) {
            throw new Error("dev_ip or host is required for FULU BambuNetwork LAN/local print methods.");
        }
        if (requiresLocalAccess && !password) {
        ...
        const submissionNames = createBambuNetworkSubmissionNames(projectName, args?.task_name !== undefined ? String(args.task_name) : undefined);
        const submissionName = submissionNames.projectName;
        const submissionTaskName = submissionNames.taskName;
        const wireProjectName = requiresLanMqttConnection ? submissionName : projectName;
        const amsMapping = stringifyBridgeJson(args?.ams_mapping_bridge ?? finalAmsMapping ?? finalAmsSlots);
        const params = {
            dev_id: devId,
            task_name: requiresLanMqttConnection
                ? submissionTaskName
                : String(args?.task_name || projectName),
            project_name: wireProjectName,
            preset_name: presetName,
            filename: threeMFPath,
            config_filename: String(args?.config_filename || threeMFPath),
            plate_index: plateIndex + 1,
            ftp_folder: String(args?.ftp_folder !== undefined
                ? args.ftp_folder
                : printModel === "p1s"
                    ? "sdcard/"
                    : ""),
            ftp_file: String(args?.ftp_file || threeMfFilename),
        ...
        const bridgeOptions = this.bridgeOptionsFromArgs(args);
        const { bridgeResult, printerAcknowledgement, printerJob } = await this.bambuNetwork.withExclusiveLifecycle(async () => {
            if (requiresLanMqttConnection) {
                await this.bambuNetwork.connectPrinter({
                    devId,
                    devIp,
                    username: String(params.username),
                    accessCode: password,
                    useSsl: Boolean(params.use_ssl_for_mqtt),
                }, bridgeOptions);
                await this.bambuNetwork.discardQueuedEvents(bridgeOptions);
                if (!this.bambuNetwork.supportsProjectFileSequenceCorrelation()) {
                    throw new Error("The BambuNetwork runtime does not advertise project_file sequence correlation. " +
                        "Apply the bundled host and open-bambu-networking patches before submitting a local print.");
                }
            }
            const currentBridgeResult = await this.bambuNetwork.callWithAgent(bridgeMethod, { client_job_id: clientJobId, params }, bridgeOptions);
            if (typeof currentBridgeResult === "object" &&
                currentBridgeResult !== null &&
                currentBridgeResult.ok === false) {
                throw new Error(`FULU BambuNetwork bridge method ${bridgeMethod} failed: ${String(currentBridgeResult.error || "unknown bridge error")}`);
            }
            if (typeof currentBridgeResult === "object" &&
                currentBridgeResult !== null &&
                typeof currentBridgeResult.value === "number" &&
                currentBridgeResult.value !== 0) {
                const value = currentBridgeResult.value;
                throw new Error(`FULU BambuNetwork bridge method ${bridgeMethod} returned non-zero result ${value}.`);
            }
            let acknowledgement;
            let printerJob;
            if (requiresBambuNetworkProjectFileAcknowledgement(bridgePrintMethod)) {
                const projectFileSequenceId = typeof currentBridgeResult === "object" &&
                    currentBridgeResult !== null &&
                    ["string", "number"].includes(typeof currentBridgeResult
                        .project_file_sequence_id)
                    ? String(currentBridgeResult
                        .project_file_sequence_id).trim()
                    : "";
                acknowledgement =
                    await this.bambuNetwork.waitForLocalPrintAcknowledgement(devId, projectFileSequenceId, bridgeOptions);
                printerJob = await this.bambuNetwork.waitForPrinterJobStart(devId, [submissionName, submissionTaskName], bridgeOptions);
            }
            return {
                bridgeResult: currentBridgeResult,
                printerAcknowledgement: acknowledgement,
                printerJob,
            };
    ...
    setupResourceHandlers() {
        this.server.setRequestHandler(ListResourcesRequestSchema, async () => {
            const configured = this.printerRegistry.list();
            const resourcePrinters = configured;
            const resources = resourcePrinters.flatMap((printer) => [
                {
                    uri: `printer://${printer.id}/status`,
                    name: `${printer.name} Status`,
                    mimeType: "application/json",
                    description: `Current status for configured printer ${printer.id}`,
                },
                {
                    uri: `printer://${printer.id}/files`,
                    name: `${printer.name} Files`,
                    mimeType: "application/json",
                    description: `Files on configured printer ${printer.id}`,
                },
                {
                    uri: `printer://${printer.id}/hms`,
                    name: `${printer.name} HMS Diagnostics`,
                    mimeType: "application/json",
                    description: `HMS diagnostics for configured printer ${printer.id}`,
                },
            ]);
            if (configured.length > 0) {
                resources.unshift({
                    uri: "fleet://status",
                    name: "Bambu Printer Fleet Status",
                    mimeType: "application/json",
                    description: "Redacted status summary for every configured printer",
                });
            }
            return {
                resources,
        ...
        this.server.setRequestHandler(ListResourceTemplatesRequestSchema, async () => {
            return {
                resourceTemplates: [
                    {
                        uriTemplate: "printer://{printer_id}/status",
                        name: "Bambu Printer Status",
                        mimeType: "application/json"
                    },
                    {
                        uriTemplate: "printer://{printer_id}/files",
                        name: "Bambu Printer Files",
                        mimeType: "application/json"
                    },
                    {
                        uriTemplate: "printer://{printer_id}/hms",
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            if (uri === "fleet://status") {
                const fleetStatus = await this.getFleetStatus(3);
                return {
                    contents: [
                        {
                            uri,
            ...
            const match = uri.match(/^printer:\/\/([^\/]+)\/(status|files|hms)$/);
            if (!match) {
                throw new McpError(ErrorCode.InvalidRequest, `Invalid resource URI: ${uri}`);
            }
            const [, printerId, resource] = match;
            const targetId = printerId === "default" && !this.printerRegistry.has("default")
                ? this.printerRegistry.targetId()
                : this.printerRegistry.targetId(printerId);
            const content = await this.printerRegistry.runExclusive(targetId, async () => {
                const profile = this.printerRegistry.resolve(targetId);
                if (resource === "status") {
                    return this.bambu.getStatus(profile.host, profile.serial, profile.accessToken);
                }
                if (resource === "files") {
                    return this.bambu.getFiles(profile.host, profile.serial, profile.accessToken);
                }
                if (resource === "hms") {
                    const status = await this.bambu.getStatus(profile.host, profile.serial, profile.accessToken);
                    let diagnostics = extractPrinterDiagnostics(status);
                    // The first MQTT status push may not include HMS data. The printer
                    // sends basic state first, then pushes HMS/error fields later.
                    if (!diagnostics.hms && status.connected) {
                        await new Promise((r) => setTimeout(r, 1500));
                        const retryStatus = await this.bambu.getStatus(profile.host, profile.serial, profile.accessToken);
    ...
    setupToolHandlers() {
        this.server.setRequestHandler(ListToolsRequestSchema, async () => {
            ...
            const tools = [
                {
                    name: "list_printers",
                    description: "List configured printer profiles without exposing hosts, serial numbers, or access tokens.",
                    inputSchema: {
                        type: "object",
                        properties: {},
                    },
                ...
                {
                    name: "add_printer",
                    description: "Add or update a named printer profile. Plaintext tokens remain in memory only; persisted profiles must reference an environment variable.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            id: { type: "string", description: "Stable lowercase printer ID used by other tools" },
                            name: { type: "string", description: "Optional operator-friendly printer name" },
                            host: { type: "string", description: "Printer hostname or LAN IP address" },
                            model: {
                                type: "string",
                                enum: [...VALID_BAMBU_MODELS],
                                description: "Exact Bambu Lab printer model",
                            },
                            serial: { type: "string", description: "Printer serial number; serial_env is preferred for persisted profiles" },
                            serial_env: { type: "string", description: "Environment variable containing the printer serial number" },
                            access_token: { type: "string", description: "LAN access token for this process only; never persisted" },
                            access_token_env: { type: "string", description: "Environment variable containing the LAN access token" },
                            dev_id: { type: "string", description: "Optional BambuNetwork device ID" },
                            dev_id_env: { type: "string", description: "Environment variable containing the BambuNetwork device ID" },
                            bed_type: { type: "string", enum: [...VALID_BED_TYPES], description: "Default build plate type" },
                            nozzle_diameter: { type: "string", enum: ["0.2", "0.4", "0.6", "0.8"], description: "Default nozzle diameter in millimeters" },
                            make_default: { type: "boolean", description: "Make this the default printer" },
                            persist: { type: "boolean", description: "Persist only nonsecret metadata and environment-variable references" },
                            replace: { type: "boolean", description: "Replace an existing profile with the same ID" },
                        },
                        required: ["id", "host", "model"],
                    },
                },
                {
                    name: "remove_printer",
                    description: "Remove a named printer profile after explicit confirmation.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            printer: printerTargetProperty(),
                            confirm: { type: "boolean", description: "Must be true to remove the profile" },
                            persist: { type: "boolean", description: "Write the updated registry to its private config file" },
                        },
                        required: ["printer", "confirm"],
                    },
                },
                {
                    name: "set_default_printer",
                    description: "Select the printer used when a tool call omits printer.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            printer: printerTargetProperty(),
                            persist: { type: "boolean", description: "Write the updated default to the private config file" },
                        },
                        required: ["printer"],
                    },
                },
                {
                    ...
                    description: "Get a redacted, failure-isolated status summary for every configured printer.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            concurrency: { type: "number", description: "Maximum simultaneous status requests, from 1 to 8 (default: 3)" },
                        },
                    },
                },
                {
                    name: "reconnect_printer",
                    description: "Disconnect and reconnect one configured printer, or all configured printers with confirm_all:true.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            printer: { type: "string", description: "Configured printer ID, or all" },
                            confirm_all: { type: "boolean", description: "Must be true when printer is all" },
                        },
                        required: ["printer"],
                    },
                },
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            probe_printer: { type: "boolean", description: "When true, complete the LAN certificate and MQTT connection lifecycle against the selected printer without starting a print." },
                            ...
                            bambu_network_cert_dir: { type: "string", description: "Directory containing slicer_base64.cer and printer.cer; defaults to BAMBU_NETWORK_CERT_DIR or an installed Bambu/Orca certificate directory." },
                            dev_id: { type: "string", description: "Bambu device id for the optional LAN printer probe; defaults to the selected printer serial." },
                            dev_ip: { type: "string", description: "Printer IP address for the optional LAN printer probe; defaults to the selected printer host." },
                            host: { type: "string", description: "Selected printer host or IP for the optional LAN printer probe." },
                            bambu_serial: { type: "string", description: "Selected printer serial for the optional LAN printer probe." },
                            bambu_token: { type: "string", description: "Selected printer access code for the optional LAN printer probe." },
                            username: { type: "string", description: "Printer username for the optional LAN printer probe; defaults to bblp." },
                            password: { type: "string", description: "Printer access code override for the optional LAN printer probe." },
                            use_ssl_for_mqtt: { type: "boolean", description: "Whether the optional LAN printer probe uses MQTT TLS; defaults to true." },
                ...
                },
                {
                    ...
                    description: "Call a diagnostic FULU OrcaSlicer-bambulab BambuNetwork bridge method, optionally with an initialized network agent. Connection, certificate, and print-start methods are reserved for the guarded readiness and print tools.",
                    inputSchema: {
                        type: "object",
                        properties: {
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            project_name: { type: "string", description: "Optional project name sent unchanged for cloud methods. Guarded LAN print methods append a unique MCP submission suffix for exact live-job confirmation." },
                            ...
                            task_name: { type: "string", description: "Optional BambuNetwork task name; defaults to the unique MCP submission name." },
                            ...
                            user_info: { type: "string", description: "Optional BambuNetwork user_info JSON string to pass to net.change_user after the agent starts." },
                            timeout_ms: { type: "number", description: "Bridge request timeout in milliseconds." },
                            confirmation_timeout_ms: { type: "number", description: "Maximum milliseconds to wait separately for the project_file response and exact live push_status confirmation; defaults to 15000." },
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            bambu_network_cert_dir: { type: "string", description: "Directory containing slicer_base64.cer and printer.cer for the FULU BambuNetwork agent." },
            ...
            for (const tool of tools) {
                if (!PRINTER_CONTEXT_TOOLS.has(tool.name))
                    continue;
                (_a = tool.inputSchema).properties ?? (_a.properties = {});
                (_b = tool.inputSchema.properties).printer ?? (_b.printer = printerTargetProperty());
                if (tool.name !== "slice_stl" &&
                    tool.name !== "slice_with_template" &&
                    Array.isArray(tool.inputSchema.required)) {
                    tool.inputSchema.required = tool.inputSchema.required.filter((field) => field !== "bambu_model");
                }
            }
            return { tools };
        });
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            const args = { ...(rawArgs ?? {}) };
            const optionalPrinterContext = name === "slice_stl" || name === "slice_with_template";
            const implicitOptionalTarget = optionalPrinterContext && typeof args.printer !== "string"
                ? this.printerRegistry.implicitReadyTargetId()
                : undefined;
            const shouldResolvePrinter = PRINTER_CONTEXT_TOOLS.has(name) &&
                (!optionalPrinterContext ||
                    typeof args.printer === "string" ||
                    implicitOptionalTarget !== undefined);
            ...
            try {
                ...
                const executeTool = async () => {
                    if (shouldResolvePrinter) {
                        const printerContext = this.resolvePrinterContext(args);
                        this.applyPrinterContext(args, printerContext);
                    }
                    ...
                    switch (name) {
                        case "list_printers":
                            result = {
                                status: "success",
                                printers: this.printerRegistry.list(),
                            };
                            break;
                        case "add_printer": {
                            const printerId = String(args.id || "").trim().toLowerCase();
                            const profileInput = {
                                id: printerId,
                                name: args.name !== undefined ? String(args.name) : undefined,
                                host: String(args.host || ""),
                                model: String(args.model || ""),
                                serial: args.serial !== undefined ? String(args.serial) : undefined,
                                serialEnv: args.serial_env !== undefined ? String(args.serial_env) : undefined,
                                accessToken: args.access_token !== undefined ? String(args.access_token) : undefined,
                                accessTokenEnv: args.access_token_env !== undefined
                                    ? String(args.access_token_env)
                                    : undefined,
                                devId: args.dev_id !== undefined ? String(args.dev_id) : undefined,
                                devIdEnv: args.dev_id_env !== undefined ? String(args.dev_id_env) : undefined,
                                bedType: args.bed_type !== undefined ? String(args.bed_type) : undefined,
                                nozzleDiameter: args.nozzle_diameter !== undefined
                                    ? String(args.nozzle_diameter)
                                    : undefined,
                                default: args.make_default === true,
                            };
                            const addProfile = async () => {
                                let previous;
                                if (args.replace === true && this.printerRegistry.has(printerId)) {
                                    try {
                                        previous = this.printerRegistry.resolve(printerId);
                                    }
                                    catch {
                                        // An unavailable profile cannot have a usable active client.
                                    }
                                }
                                const added = this.printerRegistry.add(profileInput, { persist: args.persist === true, replace: args.replace === true });
                                if (previous) {
                                    await this.bambu.disconnectPrinter(previous.host, previous.serial, previous.accessToken);
                                }
                                return added;
                            };
                            const profile = args.replace === true
                                ? await this.printerRegistry.runExclusive(printerId, addProfile)
                                : await addProfile();
                            ...
                            result = {
                                ...
                                note: args.persist === true
                                    ? "Saved nonsecret metadata and environment-variable references only."
                                    : "Registered for this MCP process only.",
                            };
                            break;
                        }
                        case "remove_printer": {
                            if (args.confirm !== true) {
                                result = {
                                    status: "skipped",
                                    removed: false,
                                    message: "remove_printer requires confirm:true.",
                                };
                                break;
                            }
                            const printerId = String(args.printer || "").trim().toLowerCase();
                            const removed = await this.printerRegistry.runExclusive(printerId, async () => {
                                let profile;
                                try {
                                    profile = this.printerRegistry.resolve(printerId);
                                }
                                catch {
                                    // A profile with unavailable credentials has no usable active client.
                                }
                                if (profile) {
                                    await this.bambu.disconnectPrinter(profile.host, profile.serial, profile.accessToken);
                                }
                                return this.printerRegistry.remove(printerId, {
                                    persist: args.persist === true,
                                });
                            });
                            await this.server.sendResourceListChanged();
                            result = { status: "success", removed: true, printer: removed };
                            break;
                        }
                        case "set_default_printer":
                            result = {
                                status: "success",
                                printer: this.printerRegistry.setDefault(String(args.printer || ""), {
                                    persist: args.persist === true,
                                }),
                            };
                            break;
                        case "get_fleet_status":
                            result = await this.getFleetStatus(Number(args.concurrency ?? 3));
                            break;
                        case "reconnect_printer": {
                            const requestedTarget = String(args.printer || "").trim().toLowerCase();
                            if (requestedTarget === "all" && args.confirm_all !== true) {
                                throw new Error('reconnect_printer with printer:"all" requires confirm_all:true.');
                            }
                            const targetIds = requestedTarget === "all"
                                ? this.printerRegistry.list().map((profile) => profile.id)
                                : [this.printerRegistry.targetId(requestedTarget)];
                            const reconnectResults = await Promise.all(targetIds.map((targetId) => this.printerRegistry.runExclusive(targetId, async () => {
                                let profile;
                                try {
                                    profile = this.printerRegistry.resolve(targetId);
                                }
                                catch {
                                    return {
                                        printer: targetId,
                                        connected: false,
                                        status: "not_ready",
                                        error: "Printer credentials are incomplete.",
                                    };
                                }
                                try {
                                    await this.bambu.disconnectPrinter(profile.host, profile.serial, profile.accessToken);
                            ...
                            result = {
                                status: reconnectResults.every((entry) => entry.connected)
                                    ? "success"
                                    : "partial",
                                printers: reconnectResults,
                            };
                            break;
                        }
                        ...
                        case "bambu_network_bridge_status": {
                            ...
                            if (Boolean(bridgeArgs?.connect) || Boolean(bridgeArgs?.probe_printer)) {
                                result = await this.bambuNetwork.withExclusiveLifecycle(async () => {
                                    ...
                                    if (Boolean(bridgeArgs?.probe_printer)) {
                                        const devId = String(bridgeArgs?.dev_id ||
                                            bridgeArgs?.bambu_serial ||
                                            bambuSerial ||
                                            "").trim();
                                        const devIp = String(bridgeArgs?.dev_ip ||
                                            bridgeArgs?.host ||
                                            host ||
                                            "").trim();
                                        const accessCode = String(bridgeArgs?.password || bridgeArgs?.bambu_token || bambuToken || "").trim();
                                        if (!devId || !devIp || !accessCode) {
                                            throw new Error("probe_printer requires a selected printer with device id, IP address, and access code.");
                                        }
                                        await this.bambuNetwork.connectPrinter({
                                            devId,
                                            devIp,
                                            username: String(bridgeArgs?.username || "bblp"),
                                            accessCode,
                                            useSsl: bridgeArgs?.use_ssl_for_mqtt !== undefined
                                                ? Boolean(bridgeArgs.use_ssl_for_mqtt)
                                                : true,
                                        }, options);
                                        probeResult = {
                                            ...probeResult,
                                            lanPrinterConnected: true,
                                        };
                                    }
                                    return probeResult;
                                });
                        ...
                        case "bambu_network_call": {
                            ...
                            const payload = bridgeArgs.payload && typeof bridgeArgs.payload === "object"
                                ...
                                : {};
                            const options = this.bridgeOptionsFromArgs(bridgeArgs);
                            const method = String(bridgeArgs.method);
                            if (isManagedBambuNetworkCallMethod(method)) {
                                throw new Error(`BambuNetwork method ${method} is managed by the readiness and print tools so its connection and acknowledgement lifecycle cannot be bypassed.`);
                            }
                            const call = async () => method === "bridge.poll_events"
                                ? await this.bambuNetwork.pollEventsForControl(payload, options)
                                : requiresBambuNetworkAgentForRawCall(method, bridgeArgs.with_agent)
                                    ? await this.bambuNetwork.callWithAgent(method, payload, options)
                                    : await this.bambuNetwork.requestControl(method, payload, options);
                            result =
                                isBambuNetworkControlMethod(method) &&
                                    this.bambuNetwork.isRawControlLaneOpen()
                                    ? await call()
                                    : await this.bambuNetwork.withExclusiveLifecycle(call, { allowRawControl: true });
                            break;
                        }
                ...
                if (shouldResolvePrinter) {
                    const targetId = implicitOptionalTarget ?? this.printerRegistry.targetId(typeof args.printer === "string" ? args.printer : undefined);
                    await this.printerRegistry.runExclusive(targetId, async () => {
                        if (targetId !== "adhoc" && typeof args.printer !== "string") {
                            args.printer = targetId;
                        }
                        await executeTool();
                    });
                }
