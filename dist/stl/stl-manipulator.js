...
export class STLManipulator extends EventEmitter {
    ...
    async sliceSTL(stlFilePath, slicerType, slicerPath, slicerProfile, progressCallback, printerPreset, bambuOptions) {
        ...
        const redactDiagnostic = (message, extraPaths = []) => {
            if (debugSlicer)
                return message;
        ...
        if (slicerProfile && !fs.existsSync(slicerProfile)) {
            console.warn(redactDiagnostic(`Slicer profile specified but not found: ${slicerProfile}. Slicer might use defaults.`));
        ...
        try {
            ...
            if (progressCallback)
                progressCallback(20, safeExecutionMessage);
            console.log(debugSlicer
            ...
            await new Promise((resolve, reject) => {
                const process = execFile(slicerPath, args, (error, stdout, stderr) => {
                    if (error) {
                        ...
                        reject(new Error(`Slicer ${slicerLabel} failed${exitCode}. ` +
                        ...
                        if (debugSlicer) {
                            ...
                            if (stderr)
                                console.warn(`Slicer Stderr: ${stderr}`);
                        }
                        else {
