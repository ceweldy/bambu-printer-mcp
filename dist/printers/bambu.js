...
export function loadClientCreds() {
    ...
    if (Boolean(explicitCertPath) !== Boolean(explicitKeyPath))
        return null;
    ...
    if (!explicitCertPath &&
        ...
        if (existsSync(legacyCertPath) && existsSync(legacyKeyPath)) {
            ...
            try {
                ...
                try {
                    if (wroteCert && existsSync(certPath))
                        unlinkSync(certPath);
                    if (wroteKey && existsSync(keyPath))
                        unlinkSync(keyPath);
                }
                catch {
...
function hasOperationalStatus(report) {
...
class BambuClientStore {
    constructor() {
        ...
        this.connectingPrinters = new Map();
    ...
    clearPrinterState(key, expectedEpoch) {
    ...
    async disconnect(host, serial, token) {
...
export class BambuImplementation {
    ...
    async disconnectPrinter(host, serial, token) {
