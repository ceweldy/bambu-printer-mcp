...
function validateModel(value) {
    ...
    if (!SUPPORTED_BAMBU_MODELS.includes(model)) {
        throw new Error(`Unsupported printer model "${model}". Supported models: ${SUPPORTED_BAMBU_MODELS.join(", ")}.`);
    }
    return model;
}
function validateProfile(raw, source, allowInlineToken) {
    const id = requiredString(raw.id, "printer id").toLowerCase();
    if (!PROFILE_ID_PATTERN.test(id)) {
        throw new Error(`Invalid printer id "${id}". Use 1-64 lowercase letters, numbers, dots, underscores, or hyphens.`);
    }
    if (id === "all") {
    ...
    if (serial && !allowInlineToken) {
        throw new Error(`Printer "${id}" contains a plaintext serial in the config file. ` +
            "Move it to an environment variable and set serialEnv instead.");
    }
    if (accessToken && !allowInlineToken) {
        throw new Error(`Printer "${id}" contains a plaintext accessToken in the config file. ` +
    ...
    if (devId && !allowInlineToken) {
        throw new Error(`Printer "${id}" contains a plaintext devId in the config file. ` +
    ...
    const accessTokenEnv = validateEnvName(optionalString(raw.accessTokenEnv), `accessTokenEnv for printer "${id}"`);
...
function parseRegistryPayload(raw, label) {
    let parsed;
    try {
...
export class PrinterRegistry {
    constructor(options = {}) {
        ...
        this.configPath = configuredPath ?? path.join(homeDir, ".config", "bambu-printer-mcp", "printers.json");
    ...
    assertNoDuplicatePhysicalPrinter(candidate) {
        const candidateSerial = candidate.serial || optionalString(candidate.serialEnv ? this.env[candidate.serialEnv] : undefined);
        if (!candidateSerial)
            return;
        for (const existing of this.profiles.values()) {
            if (existing.id === candidate.id)
                continue;
            const existingSerial = existing.serial || optionalString(existing.serialEnv ? this.env[existing.serialEnv] : undefined);
            if (existingSerial && existingSerial === candidateSerial) {
                throw new Error(`Printer profiles "${candidate.id}" and "${existing.id}" resolve to the same physical printer.`);
            }
        }
    }
    targetId(id) {
        ...
        if (normalizedId) {
            if (!this.profiles.has(normalizedId)) {
                throw new Error(`Unknown printer "${id}". Configured printers: ${this.list().map((entry) => entry.id).join(", ") || "none"}.`);
            }
            return normalizedId;
        }
        ...
        if (this.profiles.size > 1) {
            throw new Error(`Multiple printers are configured. Pass printer with one of: ${this.list().map((entry) => entry.id).join(", ")}.`);
        }
        return "adhoc";
    }
    implicitReadyTargetId() {
        const id = this.defaultPrinterId ?? (this.profiles.size === 1 ? this.profiles.keys().next().value : undefined);
        if (!id)
            return undefined;
    ...
    load() {
        ...
        const accessToken = optionalString(this.env.BAMBU_PRINTER_ACCESS_TOKEN || this.env.BAMBU_TOKEN);
    ...
    installProfiles(rawProfiles, source, allowInlineToken, defaultPrinter) {
    ...
    summarize(profile) {
        const missing = [];
        if (profile.serialEnv && !optionalString(this.env[profile.serialEnv]))
            missing.push(profile.serialEnv);
        if (profile.accessTokenEnv && !optionalString(this.env[profile.accessTokenEnv])) {
            missing.push(profile.accessTokenEnv);
        }
        if (profile.devIdEnv && !optionalString(this.env[profile.devIdEnv]))
            missing.push(profile.devIdEnv);
        const serialReady = Boolean(profile.serial || (profile.serialEnv && !missing.includes(profile.serialEnv)));
        const tokenReady = Boolean(profile.accessToken || (profile.accessTokenEnv && !missing.includes(profile.accessTokenEnv)));
        return {
            id: profile.id,
    ...
    resolve(id, overrides = {}) {
        ...
        if (!profile) {
            if (!overrides.host || !overrides.serial || !overrides.accessToken || !overrides.model) {
                throw new Error("No ready printer is configured. Configure BAMBU_PRINTERS_JSON/BAMBU_PRINTERS_FILE, " +
        ...
        const serial = optionalString(overrides.serial) || profile.serial || optionalString(profile.serialEnv ? this.env[profile.serialEnv] : undefined);
        const accessToken = optionalString(overrides.accessToken) || profile.accessToken || optionalString(profile.accessTokenEnv ? this.env[profile.accessTokenEnv] : undefined);
        ...
        const devId = optionalString(overrides.devId) ||
            profile.devId ||
            optionalString(profile.devIdEnv ? this.env[profile.devIdEnv] : undefined) ||
            serial;
        const missing = [];
        if (!serial)
            missing.push(profile.serialEnv || "serial");
        if (!accessToken)
            missing.push(profile.accessTokenEnv || "access token");
        if (missing.length > 0) {
        ...
        if (profile.id !== "adhoc") {
            for (const other of this.profiles.values()) {
                if (other.id === profile.id)
                    continue;
                const otherSerial = other.serial || optionalString(other.serialEnv ? this.env[other.serialEnv] : undefined);
                if (otherSerial && otherSerial === serial) {
                    throw new Error(`Printer profiles "${profile.id}" and "${other.id}" resolve to the same physical printer.`);
                }
            }
        }
        return {
    ...
    add(rawProfile, options = {}) {
        ...
        if (options.persist && (!profile.serialEnv || !profile.accessTokenEnv)) {
            throw new Error("Refusing to persist runtime-only printer identifiers or credentials. " +
        ...
        if (options.persist && profile.devId && !profile.devIdEnv) {
            throw new Error("Refusing to persist an inline BambuNetwork device identifier. " +
                "Set dev_id_env and provide the value through that environment variable.");
        }
        this.assertNoDuplicatePhysicalPrinter(profile);
        this.profiles.set(profile.id, profile);
        if (profile.default || this.profiles.size === 1)
            this.defaultPrinterId = profile.id;
        try {
            if (options.persist)
                this.persist();
            ...
            if (previousProfile)
                this.profiles.set(profile.id, previousProfile);
            else
    ...
    remove(id, options = {}) {
        ...
        if (!profile)
            throw new Error(`Unknown printer "${id}".`);
    ...
    async runExclusive(printerId, operation) {
        ...
        try {
            ...
            if (this.commandTails.get(printerId) === current)
                this.commandTails.delete(printerId);
        }
    }
    persist() {
        ...
        if (runtimeOnlyProfiles.length > 0) {
            throw new Error(`Cannot persist the registry while these profiles lack serial_env or access_token_env references: ${runtimeOnlyProfiles.join(", ")}.`);
        }
        const inlineDeviceIds = [...this.profiles.values()]
            .filter((profile) => profile.devId && !profile.devIdEnv)
            .map((profile) => profile.id);
        if (inlineDeviceIds.length > 0) {
            throw new Error(`Cannot persist the registry while these profiles lack dev_id_env references: ${inlineDeviceIds.join(", ")}.`);
        }
        const directory = path.dirname(this.configPath);
        const directoryExisted = fs.existsSync(directory);
        fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
        if (!directoryExisted || this.ownsConfigDirectory)
            fs.chmodSync(directory, 0o700);
        const printers = [...this.profiles.values()].map((profile) => ({
            id: profile.id,
            name: profile.name,
            host: profile.host,
            model: profile.model,
