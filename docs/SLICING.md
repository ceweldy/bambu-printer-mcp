...
| Pre-sliced `.gcode.3mf` → printer | MCP `print_3mf` | ✅ Supported on verified H2-family workflows |
...
- **Multi-color slicing must use the GUI.** Open the model in Bambu Studio,
  paint or split-and-assign filaments, export the sliced `.gcode.3mf`, then
  hand it to MCP `print_3mf`.
